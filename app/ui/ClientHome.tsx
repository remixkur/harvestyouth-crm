            ...
            {activePage === "growth" && (
              <div className="space-y-5">
                ...
                <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
                  ...
                  <div className="divide-y divide-slate-100">
                    {people.filter((p) => !p.archived && !p.full_course).map((person) => (
                      <div
                        key={person.id}
                ...
                <div className="grid gap-4">
                  {archivedPeople.map((person) => (
                    ...
                    >
                      ...
                      <div className="mt-2 text-sm text-slate-500">Наставник: {person.mentor_name || "—"}</div>
                      <div className="mt-3 text-sm text-slate-700">{person.archive_reason || person.comment || "—"}</div>
                    </div>
                  ))}
